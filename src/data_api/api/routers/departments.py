...
@router.get("/search", response_model=dict[str, Union[list[schemas.Department], list[schemas.DepartmentPerson]]])
async def fuzzy_search_departments_and_people(response: Response, query: str = Query(..., example="校長")):
    """Fuzzy search departments and people."""
    commit_hash, data = await services.departments_service.fuzzy_search_departments_and_people(query=query)
    if commit_hash is None:
        raise HTTPException(status_code=503, detail="Service temporarily unavailable")
    response.headers["X-Data-Commit-Hash"] = commit_hash
    return data
